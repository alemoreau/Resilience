...
\section{Methodology}
In the following, several definitions and assumptions are given to define the models and ensure the experiment reproducibility.
...
requires to compute the residual vector at an extra cost of a matrix-vector product. In exact arithmetic these two vectors are equal,
...
The evaluation focuses on transient faults that numerically affects the GMRES executions. Other errors introduced by transient faults such as memory access error or instruction modifications are supposed to be separately taken care of by low overhead techniques not described here. Hence, faults are modeled by a perturbation happening in a register which contains numerical data. Several types of perturbation can occur, but the fault model we define only uses bit-flips for simplicity. Moreover, it is assumed that no more than one fault may occur during the whole GMRES execution, as the occurrence of a fault is assumed to be a rare enough event compared to the execution time, and we will leave the two or more faults situation to the discussion section. Since the most expensive operation is the sparse-matrix vector product  (line 6 in Algorithm~\ref{alg:gmres}), it is assumed that the fault may only occur at this moment for at least two reasons: on the one hand, most of the execution time will be spent performing this operation, which directly increases the probability of a fault happening there, and on the other hand, remaining operations can be duplicated without any significant impact on the execution time, enabling easy fault detection and error correction for those parts of the algorithm. Algorithm \ref{alg:faulty_product} is equivalent to the faulty product used in the following experiments. To simplify, it does not use any particular data structure and is based on the standard dense matrix-vector product. Hence it consists in two nested loops and we define the location $(i, k)$ where the fault occurred as the indexes of those loops at the moment of the fault. The operation repeated in the inner loop involves three registers, $reg_1$ stores $A_{i, k}$, $reg_2$ stores $v_k$ and $reg_3$ stores $A_{i, k} \cdot v_k$. In the numerical experiments, double precision is used so those registers contains 64 bits that could be affected by one bit-flip.
...
For the first part, the matrices used are HB/gre_216a (Figure~\ref{fig:gre_216a}), chosen for its small size ($n=216$) and handy convergence history, which will be used to illustrate the GMRES algorithm results, and HB/pores_2 (Figure~\ref{fig:pores_2}), which is bigger ($n=1224$) and will be used with the preconditioned-GMRES algorithms. The results will be put in parallel to illustrate the common behavior of those algorithms when they are subject to soft errors. When left-preconditioned-GMRES and right-preconditioned-GMRES present the same behavior, only the result of right-preconditioned-GMRES are displayed.  For the second part, the matrices used are gathered in Table \ref{table:nonlin}.
...
\begin{figure}[!ht]
  ...
  The second way used to represent an execution is simply by its outcome. For instance, Table~\ref{table:outcomes} represents a set of outcomes used later in this report. If the reference non faulty execution requires $\ell_{ref}$ iteration to converge, we define the convergence without delay as the convergence of a corresponding faulty execution in exactly $\ell_{ref}$ iterations, the convergence with delay as the convergence in between $\ell_{ref}+1$ iterations and $2\ell_{ref}-1$ iterations and no convergence when the convergence was not reached after $2\ell$ iterations.


  \begin{table}[h]
    \centering
    \caption{Color used for each test outcome. $\ell$ (resp. $\ell_{ref}$) is the number of iterations for the faulty execution (resp. reference non faulty execution) to converge.}
    \label{table:outcomes}
    \begin{tabular}{|c|c|}
      \hline
      Outcome  & $\ell$ \tabularnewline
      \hline
      \color[RGB]{50, 150, 50}{\textbf{Convergence without delay}} & $\ell = \ell_{ref}$ \\
      \color{blue}{\textbf{Convergence with delay}} & $\ell_{ref} < \ell < 2\ell_{ref}$\\
      \color{red}{\textbf{No convergence}} & $\ell \geq 2\ell_{ref}$ \\
...
%   \subsubsection{Qualitative results and fault parameters influence}
  % To visualize the influence of each fault parameter, the convergence history of three disrupted GMRES executions over the same matrices (HB/gre_216a for GMRES and HB/pores_2 for preconditioned-GMRES) are plotted for each one of the four fault parameters (bit flipped, iteration, register and location) while the others are fixed. Since both variants of preconditioned-GMRES give similar results, only the right-preconditioned-GMRES ones are displayed.
  ...
  %   \end{minipage}
  %   \caption{Convergence history of three GMRES executions on gre_216a and three preconditioned-GMRES executions on pores_2, disrupted by a transient fault. In the execution \ref{fig:gre_216a_conv_hist_bit_0}, the fault (\textbf{bit 0}) disrupts the convergence, in \ref{fig:gre_216a_conv_hist_bit_1}, the fault (\textbf{bit 25}) delays it, and in \ref{fig:gre_216a_conv_hist_bit_2}, the fault (\textbf{bit 50}) has no effect on the convergence. In both cases, bit-flips on the most significant bits have a larger impact than bit-flips on the least significant bits. }\label{fig:conv_hist_bit}
  % \end{figure}
  ...
  % The remaining fault parameters (register and location) do not appear to have any significant impact on the convergence, even though inputs can be constructed to contradict this observation. Figure \ref{fig:conv_hist_register}, representing the convergence history of three GMRES executions and three preconditioned-GMRES disrupted by a bit-flip on $reg_1$, $reg_2$ and $reg_3$, and Figure \ref{fig:conv_hist_location}, representing the convergence history of three GMRES executions and three preconditioned-GMRES disrupted by a bit-flip on three different locations, present similar behaviors in terms of convergence history.
  ...
  %   \end{minipage}
  %   \caption{Convergence history of three GMRES executions on gre_216a and 3 preconditioned-GMRES executions on pores_2, disrupted by a transient fault. In all the executions, the faults (occurring in $reg_1$, $reg_2$ and $reg_3$) seem to have the same impact.}\label{fig:conv_hist_register}
  % \end{figure}
  ...
  %   \end{minipage}
  %   \caption{Convergence history of three GMRES executions on gre_216a and three preconditioned-GMRES executions on pores_2, disrupted by a transient fault. In all the executions, the faults (occurring in random locations) seem to have the same impact.}\label{fig:conv_hist_location}
  % \end{figure}
...
%  \newpage
  ...
  In the previous experiments, we showed that a transient fault may have a significant impact on the convergence of an execution. In the following, an analysis is proposed to evaluate this impact using the theory of inexact GMRES, in order to predict the convergence behavior in Section~\ref{sec:prediction}.
  ...
  The residual gap provides a good quantification of the fault impact, however it does not explain nor quantifies by how much the convergence is delayed (when convergence occurs).
  ...
  \end{figure}\label{fig:conv_hist_true_delta}
  ...
  \begin{figure}[!ht]
    ...
    \end{subfigure}
    \caption{Two convergence histories disrupted by a fault. The light blue and purple lines represent the measured residual gap ($\|\widetilde{r}_{i} - \widetilde{r}_{i}\textprime \|$) and the computed residual gap ($|\widetilde{y}_{{i}, f}|\cdot \|{w}_{f+1} - \widetilde{w}_{f+1}\|$) evolutions in the faulty executions: GMRES on gre_216a (\ref{fig:gre_216a_conv_hist_delta}) and preconditioned-GMRES  on pores_2  (\ref{fig:pores_2_conv_hist_delta}).}
  \end{figure}

  \subsubsection{Prediction of the fault impact}\label{sec:prediction}
  In Section~\ref{sec:empirical}, we observed that faults may either have a critical impact when preventing the execution from converging, little impact when delaying the convergence, or no impact at all. Ideally, by using the symptoms brought by a fault we should be able to predict its impact.
  ...
  \begin{proof}
    ...
    Since the GMRES execution terminates in $\ell$ iterations for the reduced target accuracy $(1 - c) \varepsilon$, we have: $$\|\widetilde{r}\textprime_{\ell}\| \leq (1 - c) \varepsilon \text{.}$$
    Then, using Equation \eqref{eqn:delta} derived from \cite{DBLP:journals/siamsc/GiraudGL07} and the criterion $\|{w}_{f+1} - \widetilde{w}_{f+1}\| < \frac{c \cdot \varepsilon}{|\widetilde{y}_{\ell, f}|}$, we have :
    \begin{equation}
      % 
      \begin{split}
  ...
  If the error introduced by the fault is lower than the threshold, the execution must converge (and then is predicted to converge) to the target accuracy $\varepsilon$ according to Theorem \ref{theorem}. Note that the reverse is not necessarily true so if the error is greater than the threshold, the execution may or may not converge to the target accuracy (no prediction). Figure~\ref{fig:compared} illustrates this idea. It displays the convergence histories of four executions disrupted by a fault. At the fault iteration $f$, the error is compared to the threshold. In the upper plots, the error is lower than the threshold, hence the execution is predicted to converge. On the contrary in the bottom plots, the error is greater than the threshold so no convergence prediction can be made (although in both case the executions happen to converge anyway). For the prediction, the value $c=0.5$ was used.
  ...
  \begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Prediction of the convergence in several executions disrupted by a fault. Each dot corresponds to an execution, its abscissa is the bit flipped, its ordinate is the fault injection time in the execution, and its color represents whether the execution was predicted to converge (green) or no prediction was made (red). The executions are split into three plots for each matrix used, one for each convergence outcome (convergence, delayed convergence and no convergence). For the prediction, the value $c=0.5$ was used. Every execution that was predicted to converged did  (Theorem~\ref{theorem}) since all of them are located in the upper four plots (convergence or delayed convergence).}\label{fig:prediction}
  \end{figure}
  ...
  Table \ref{table:theoretical_outcomes} gathers the four possible outcomes produced by this detection scheme. If the execution converges, the fault is said to have no impact, whereas a fault preventing the execution from converging is said to be critical.   
  \begin{table}[!ht]
    \centering
  ...
  Figure \ref{fig:compared} can be reinterpreted in terms of detection. In figures \ref{fig:gre_216a_conv_hist_prediction} and 
  \ref{fig:pores_2_conv_hist_prediction}, no fault is detected as the error is smaller than the threshold, and the executions converge, producing a \emph{No impact fault ignored} from Table~\ref{table:theoretical_outcomes}. In figures \ref{fig:gre_216a_conv_hist_no_prediction} and 
  \ref{fig:pores_2_conv_hist_no_prediction}, a critical fault is detected as the error is greater than the threshold $\tau_{0.5}$ at the fault iteration and the executions does not converge, producing a \emph{Critical fault detected} from Table~\ref{table:theoretical_outcomes}. In both cases, the fault is properly detected.
  ...
  \begin{figure}[!ht]
    ...
    \begin{minipage}[b]{0.48\linewidth}
      ...
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/gre_216a/figure\thegrecounter.pdf}\stepcounter{grecounter})
        \caption{$\varepsilon = 10^{-5}$}\label{fig:gre_216a_conv_hist_theoretical_detection_0}	
      \end{subfigure}
      \quad
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/gre_216a/figure\thegrecounter.pdf}\stepcounter{grecounter})
        \caption{$\varepsilon = 10^{-10}$}\label{fig:gre_216a_conv_hist_theoretical_detection_1}	
      \end{subfigure}
    \end{minipage}
    \quad
    \begin{minipage}[b]{0.48\linewidth}
      ...
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/pores_2/right/figure\theporescounter.pdf}\stepcounter{porescounter}
        \caption{$\varepsilon = 10^{-5}$}\label{fig:pores_2_conv_hist_theoretical_detection_0}	
      \end{subfigure}
      \quad
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/pores_2/right/figure\theporescounter.pdf}\stepcounter{porescounter}
        \caption{$\varepsilon = 10^{-10}$}\label{fig:pores_2_conv_hist_theoretical_detection_1}	
      \end{subfigure}

    \end{minipage}
    \caption{Convergence histories of a GMRES execution on HB/gre_216a and a right-preconditioned-GMRES executions on HB/pored_2 for two target accuracies ($\varepsilon = 10^{-5}$ and $\varepsilon = 10^{-10}$). }\label{fig:conv_hist_detection_theoretical}
  \end{figure}
 ...
 The oracle-based detection scheme described in Section~\ref{sec:detection_oracle} manages to correctly detect faults in most cases, however, it cannot be implemented for several reasons. First, the error $\|{w}_{f+1} - \widetilde{w}_{f+1}\|$ cannot be measured directly, since $w_{f+1}$ is not available, only $\widetilde{w}_{f+1}$ is. An approximation for the error is proposed and evaluated in Section~\ref{sec:checksum}. Second, the threshold $\tau_c = c \cdot \varepsilon / |\widetilde{y}_{\ell, f}|$ requires the $\widetilde{y}_{\ell, f}$ vector which is computed at the last iteration $\ell$ of the execution, and is then not yet available at the fault iteration, so an approximation is also proposed and evaluated in Section~\ref{sec:threshold}. Moreover, the iteration when the fault occurred is not known either, but this issue can easily be taken care of by applying the scheme at each iteration, and replacing $f$ by the current iteration index $i$ as described in Section~\ref{sec:practical_scheme}. Finally, the new implementable scheme is evaluated in Section~\ref{sec:implementable_evaluated}.
  ...
  \begin{table}[h]
    ...
    \end{tabular}
  \end{table}
  In Figure \ref{fig:conv_hist_detection_correct}, the approximated threshold $\tau_{0.5}$ and the check-sum are plotted at each iteration for 2 GMRES executions disrupted by a fault. In Figure \ref{fig:gre_216a_conv_hist_detection_correct_0}, the check-sum at the fault iteration is smaller than the threshold, so the fault is not detected, and the execution does converge, which produce a \emph{No impact fault ignored} from Table~\ref{colors}. In Figure~\ref{fig:gre_216a_conv_hist_detection_correct_2}, the check-sum is greater than the threshold at the fault iteration and the execution does not converge, which produce a \emph{Critical fault detected} from Table~\ref{colors}. 
  However in Figure \ref{fig:conv_hist_detection_incorrect}, two convergence histories of executions disrupted by an incorrectly detected fault are plotted for GMRES and preconditioned-GMRES. In Figure \ref{fig:gre_216a_conv_hist_detection_incorrect_3}, check-sum(i) remains below $\tau_{0.5}(i)$ so the fault is undetected, even though the execution does not converge, which correspond to a \emph{Critical fault not detected} from Table \ref{colors}. In Figure \ref{fig:gre_216a_conv_hist_detection_incorrect_4}, check-sum(i) exceeds $\tau_{0.5}(i)$ at the fault iteration so a fault is detected, but the execution still converges which correspond to a \emph{No impact fault detected} from Table \ref{colors}. 
  \begin{figure}[!ht]
    \centering
    ...
    \begin{minipage}[b]{0.48\linewidth}
      ...
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/gre_216a/figure\thegrecounter.pdf}\stepcounter{grecounter})
        \caption{No impact fault ignored}\label{fig:gre_216a_conv_hist_detection_correct_0}
      \end{subfigure}
      \quad
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/gre_216a/figure\thegrecounter.pdf}\stepcounter{grecounter})
        \caption{Critical fault detected}\label{fig:gre_216a_conv_hist_detection_correct_2}
      \end{subfigure}
    \end{minipage}
    \quad
    \begin{minipage}[b]{0.48\linewidth}
      ...
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/pores_2/right/figure\theporescounter.pdf}\stepcounter{porescounter}
        \caption{No impact fault ignored}\label{fig:pores_2_conv_hist_detection_correct_0}
      \end{subfigure}
      \quad
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/pores_2/right/figure\theporescounter.pdf}\stepcounter{porescounter}
        \caption{Critical fault detected}\label{fig:pores_2_conv_hist_detection_correct_2}
      \end{subfigure}

      
    \end{minipage}
    \caption{Convergence histories of 2 GMRES executions on HB/gre_216a and 2 preconditioned-GMRES executions on HB/pored_2, disrupted by transient faults that are properly detected. The check-sum and the threshold at each iteration are plotted. In figures~\ref{fig:gre_216a_conv_hist_detection_correct_0} and ~\ref{fig:pores_2_conv_hist_detection_correct_0}, the check-sum at each iteration remains below the threshold, so no fault is detected and the executions converge, producing a \emph{No impact fault ignored} from Table \ref{colors} in both cases. In figures~\ref{fig:gre_216a_conv_hist_detection_correct_2} and ~\ref{fig:pores_2_conv_hist_detection_correct_2}, the check-sum becomes higher than the threshold at the fault iteration, so the faults are detected and the executions do not converge, producing a \emph{Critical fault detected} from Table \ref{colors} in both cases. }\label{fig:conv_hist_detection_correct}
  \end{figure}
  ...
  \begin{figure}[!ht]
    ...
    \begin{minipage}[b]{0.48\linewidth}
      ...
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/gre_216a/figure\thegrecounter.pdf}\stepcounter{grecounter})
        \caption{Critical fault ignored}\label{fig:gre_216a_conv_hist_detection_incorrect_3}
      \end{subfigure}
      \quad
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/gre_216a/figure\thegrecounter.pdf}\stepcounter{grecounter})
        \caption{No impact fault detected}\label{fig:gre_216a_conv_hist_detection_incorrect_4}
      \end{subfigure}
    \end{minipage}
    \quad
    \begin{minipage}[b]{0.48\linewidth}
      ...
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/pores_2/right/figure\theporescounter.pdf}\stepcounter{porescounter}
        \caption{Critical fault ignored}\label{fig:pores_2_conv_hist_detection_incorrect_3}
      \end{subfigure}
      \quad
      \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=1.15\linewidth]{figures/pores_2/right/figure\theporescounter.pdf}\stepcounter{porescounter}
        \caption{No impact fault detected}\label{fig:pores_2_conv_hist_detection_incorrect_4}
      \end{subfigure}

      
    \end{minipage}
    \caption{Convergence histories of 2 GMRES executions on HB/gre_216a and 2 preconditioned-GMRES executions on HB/pored_2, disrupted by transient faults that are not properly detected. In figures~\ref{fig:gre_216a_conv_hist_detection_incorrect_3} and ~\ref{fig:pores_2_conv_hist_detection_incorrect_3}, the check-sum at each iteration remains below the threshold, so no fault is detected and the executions do not converge, producing a \emph{Critical fault ignored} from Table \ref{colors} in both cases. In figures~\ref{fig:gre_216a_conv_hist_detection_incorrect_4} and ~\ref{fig:pores_2_conv_hist_detection_incorrect_4}, the check-sum becomes slightly higher than the threshold at the fault iteration, so the faults are detected but the executions converge anyway, producing a \emph{No impact fault detected} from Table \ref{colors} in both cases. }\label{fig:conv_hist_detection_incorrect}
  \end{figure}
...
To visualize the influence of each fault parameter, the convergence history of three disrupted GMRES executions over the same matrices (HB/gre_216a for GMRES and HB/pores_2 for preconditioned-GMRES) are plotted for each one of the four fault parameters considered (bit flipped, iteration, register and location) while the others are fixed. Since both variants of preconditioned-GMRES give similar results, only the right-preconditioned-GMRES ones are displayed.
  ...
  \begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Convergence history of three GMRES executions on gre_216a and three preconditioned-GMRES executions on pores_2, disrupted by a transient fault. In the execution \ref{fig:gre_216a_conv_hist_iteration_0}, the fault (\textbf{iteration 10}) disrupts the convergence, in \ref{fig:gre_216a_conv_hist_iteration_1}, the fault (\textbf{iteration 60}) delays it, and in \ref{fig:gre_216a_conv_hist_iteration_2}, the fault (\textbf{iteration 110}) has no effect on the convergence. In both cases, the fault impact decreases as the iteration when it is injected increases.}\label{fig:conv_hist_iteration}
  \end{figure}
  ...
  \begin{figure}[!ht]
    ...
    \end{minipage}
    \caption{Convergence history of three GMRES executions on gre_216a and three preconditioned-GMRES executions on pores_2, disrupted by a transient fault. In all the executions, the faults (occurring in $reg_1$, $reg_2$ and $reg_3$) seem to have the same impact.}\label{fig:conv_hist_register}
  \end{figure}
